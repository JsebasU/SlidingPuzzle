...
public class GameManager : MonoBehaviour {
    ...
    [SerializeField]    TextMeshProUGUI     publicHelpText          = null;
    ...
    public bool publichelpfinish = false;
    public bool publichelpfinishDoble = false;
    bool flagAudio = false;

    private             bool                IsFinished
    {
        get
        {
            return (FinishedQuestions.Count < PlayerPrefs.GetInt("canridadPreguntas")) ? false : true;
        }
    }
    ...
    IEnumerator StartTimer()
    {
        ...
        while (timeLeft > 0)
        {
            if (!pause && UIManager.firstCheckCorrect)
            {
                timeLeft--;
    ...
    public void Pause(GameObject boton)
    {
        pause = true;
        StartCoroutine(PublicHelp());
        boton.SetActive(false);
    }

    IEnumerator PublicHelp()
    {
        publichelpfinishDoble = true;
        int tiempo = PlayerPrefs.GetInt("tiempoAyudaPublico");
        if (!publichelpfinish)
        {
            while (tiempo > 0 && !publichelpfinish)
            {
                tiempo--;
                AudioManager.Instance.PlaySound("CountdownSFX");

                if (tiempo < 5 / 2 && tiempo > 5 / 4)
                {
                    publicHelpText.color = timerHalfWayOutColor;
                }
                if (tiempo < 5 / 4)
                {
                    publicHelpText.color = timerAlmostOutColor;
                }

                publicHelpText.text = tiempo.ToString();
                yield return new WaitForSeconds(1.0f);
            }
            pause = false;
            publicHelpText.text = "";
        }
        else if(publichelpfinish)
        {
            pause = false;
            publicHelpText.text = "";
        }
    ...
    public void FiftyFifty(GameObject boton)
    {
        int rCorrecta = Questions[currentQuestion].RespuestaCorrecta();
        //Debug.Log("Respuesta correcta => " + rCorrecta);
        ...
        while (flag)
        {
            ...
            if (Randoms.Count == 2)
            {
                //Debug.Log("Respuestas a desactivar => " + Randoms[0] + " - " + Randoms[1]);
                flag = false;
            }
        }
        AnswerContent.transform.GetChild(Randoms[0]).GetComponent<UnityEngine.UI.Button>().interactable = false;
        AnswerContent.transform.GetChild(Randoms[1]).GetComponent<UnityEngine.UI.Button>().interactable = false;
        /*AnswerContent.transform.GetChild(Randoms[0]).gameObject.SetActive(false);
        AnswerContent.transform.GetChild(Randoms[1]).gameObject.SetActive(false);*/
